...
> Aleksei-facing items (data entry, configuration, demo prep) live in
> `Alex-ToDo.md`. This file is dev-side TODOs only.

## Coordinator-dashboard demo data — seeded 2026-04-30

Shipped a demo-shaped seed for the Tier-3 dashboard panels —
`data/seed/coordinator-demo/`. After loading via `seed_populations`
then `seed_demo_coordination`, all seven panels render non-empty
(9 institutions, 18 populations, 8 programs, 2 transfers,
7 recommendations, 24 events). README documents swap-in for real
ZIMS numbers.

The corresponding "Aleksei-facing" walkthroughs (FR approval pass,
language-compare walkthrough, real-data swap-in checklist) live in
`Alex-ToDo.md` §2.1, §2.11.
