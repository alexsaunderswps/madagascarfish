...
the repo and loaded into local + staging. It populates all seven
panels (9 real partner institutions, 18 illustrative populations) so
the dashboard isn't visually empty if §2.1 entries don't all land
before ABQ. **It is demo-shaped, not field data** — flagged that way
in the README. Real CARES entry below still happens; the seed is a
backstop, not a substitute. When you start entering real keepers,
delete the matching demo rows or the panels will double-count.
...
That prints the English source + the three translated versions back
to back. If the locale fallback badge fires for any of them
